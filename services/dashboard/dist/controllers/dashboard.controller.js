...
exports.getOccupancyByBarber = getOccupancyByBarber;
exports.getRevenueEstimate = getRevenueEstimate;
...
async function getDashboardStats(req, res, next) {
    try {
        ...
        queries.push(q9);
        const [totalResult, todayResult, weekResult, monthResult, cancelledResult, noShowResult, completedResult, barbersResult, servicesResult] = await Promise.all(queries);
        ...
        let servicesQuery = supabase
            ...
            .gte('appointment_date', monthStart.toISOString().split('T')[0]);
        if (branch_id)
            servicesQuery = servicesQuery.eq('branch_id', branch_id);
        ...
        let barbersQuery = supabase
            ...
            .gte('appointment_date', monthStart.toISOString().split('T')[0]);
        if (branch_id)
            barbersQuery = barbersQuery.eq('branch_id', branch_id);
        const { data: barberData } = await barbersQuery;
        const barberCounts = {};
        barberData?.forEach((a) => {
...
async function getAppointmentsByDay(req, res, next) {
    try {
        ...
        for (const appt of data ?? []) {
            ...
            if (!byDay[d])
                byDay[d] = { total: 0, completed: 0, cancelled: 0 };
            byDay[d].total++;
...
async function getOccupancyByBarber(req, res, next) {
    try {
...
async function getRevenueEstimate(req, res, next) {
    try {
        ...
        for (const appt of data ?? []) {
            ...
            if (!byService[key])
                byService[key] = { name: svc.name, count: 0, revenue: 0 };
            byService[key].count++;
